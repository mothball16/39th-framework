...
function ItemEquipper.new(itemProviders: { [string]: Types.ClassItemProvider }, classes: { [string]: Types.Class })
...
function ItemEquipper:GetProvider(itemArgs: any): Types.ClassItemProvider
...
function ItemEquipper:AssignClassItems(player: Player, classId: string)
...
function ItemEquipper:UnassignClassItems(player: Player, classId: string)
	local classConfig = self.classes[classId]
	if not classConfig then
		warn(`class config not found for class {classId}`)
		return
