...
local function SimulateCast(cast: ActiveCast, delta: number, expectingShortCall: boolean)
	...
	--<DTS> DTS DRAGOON TANK SYSTEM 1.2.0 FUZE COMPAT
	if cast.UserData.FuzeTriggered and cast.UserData.FuzeTriggered==true then
		PrintDebug("Fuze explosion was successful. Terminating.")
		
		local fakeResult = { -- Convert the RaycastResult into a generic dictionary, events don't like RaycastResults for some reason
			Position = point,
			Normal = normal or rayDir,
			Material = material,
			Instance = part
		}
		
		SendRayHit(cast, resultOfCast or fakeResult, segmentVelocity, cast.RayInfo.CosmeticBulletObject)
		cast:Terminate()
