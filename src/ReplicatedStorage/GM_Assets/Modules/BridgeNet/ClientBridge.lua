	...
	RunService.Heartbeat:Connect(function()
		...
		for _, v: sendPacketQueue in SendQueue do
			...
			elseif v.requestType == "send" then
				...
				-- bridgeObj is sometimes nil for some reason?
				if bridgeObj and not (#bridgeObj._outboundMiddleware == 0) then
					local result
...
function ClientBridge:Destroy()
	...
	for k, v in self do
		-- Numbers can't be destroyed silly - InabaTwi
		if typeof(v) ~= "number" and v.Destroy ~= nil then
			v:Destroy()
		else
			self[k] = nil
		end
